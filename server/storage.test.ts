...
import { createStorage, ResilientStorage } from "./storage";
...
function bookingData(bookingNumber: string): typeof bookings.$inferInsert {
  return {
    bookingNumber,
...
test("production does not hide a missing bookings table in memory", async () => {
  ...
  const storage = new ResilientStorage(missingTableDb, false);

  await assert.rejects(
    () => storage.createBooking(bookingData("ON-2026-1002")),
...
test("development can fall back to memory when bookings table is missing", async () => {
  ...
  const storage = new ResilientStorage(missingTableDb, true);

  const booking = await storage.createBooking(bookingData("ON-2026-1003"));

  assert.equal(booking.bookingNumber, "ON-2026-1003");
  assert.ok(booking.id);
});
