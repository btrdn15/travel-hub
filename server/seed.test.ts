...
describe("secureAdminAccounts", () => {
  ...
  it("does not overwrite existing private passwords", async () => {
    const store = new TestAdminStore();
  ...
  it("creates an explicit bootstrap super admin with a private password", async () => {
    ...
    assert.equal(
      await comparePasswords("private-admin-password", owner.password),
      true,
    );
    assert.equal(await comparePasswords("admin123", owner.password), false);
  });

  it("updates an explicit bootstrap admin that still has the previous default password", async () => {
    const store = new TestAdminStore();
    store.users.set("admin1", {
      id: "admin1",
      username: "admin1",
    ...
    assert.deepEqual(
      store.updated.map((item) => item.id),
      ["admin1"],
    );
    assert.equal(
      await comparePasswords("private-admin-password", store.users.get("admin1")!.password),
      true,
    );
  });
