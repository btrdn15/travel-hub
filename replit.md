...
## Admin Accounts
- No default admin credentials are created automatically.
- To create the first super admin in a fresh environment, start the server once with
  `BOOTSTRAP_ADMIN_USERNAME` and `BOOTSTRAP_ADMIN_PASSWORD` set to strong, private values.
- Legacy `admin1`-`admin5` accounts that still use the old `admin123` password are locked on startup.
